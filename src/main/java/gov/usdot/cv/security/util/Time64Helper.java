...
public class Time64Helper {
	...
	public static Date time64ToDate(Time64 time64) {
		assert(time64.bigIntegerValue().compareTo(BigInteger.ZERO) != -1);
		
		long microToMilliseconds = time64.bigIntegerValue().longValue() / 1000;
	...
	public static Time64 dateToTime64(Date date) {
		...
		return new Time64(BigInteger.valueOf(milliToMicroseconds));
	}
