...
public class FileCertificateStore {
	...
	public static boolean load(CryptoProvider cryptoProvider, String name, String certificateFileName,
		...
		try {
			if(PrivateResourceLoader.isPrivateResource(certificateFileName)) {
				InputStream fis = PrivateResourceLoader.getFileAsStream(certificateFileName);
				certificateBytes = IOUtils.toByteArray(fis);
			}
			else {
		...
		if(privateKeyReconstructionFileName == null  && seedPrivateKeyFileName == null) {
			...
			try {
				if(PrivateResourceLoader.isPrivateResource(privateKeyReconstructionFileName)) {
					InputStream fis = PrivateResourceLoader.getFileAsStream(privateKeyReconstructionFileName);
					privateKeyReconstructionValueBytes = IOUtils.toByteArray(fis);
				}
				else {
			...
			try {
				if(PrivateResourceLoader.isPrivateResource(seedPrivateKeyFileName)) {
					InputStream fis = PrivateResourceLoader.getFileAsStream(seedPrivateKeyFileName);
					seedPrivateKeyBytes = IOUtils.toByteArray(fis);
				}
				else {
